...
当 Loading 结束之后，我们往往需要显示真实的 UI，可以通过 `loading` 的值来控制是否显示真实的 DOM。然后通过具名 Slot 来设置当 loading 结束之后需要展示的 UI。
...
1. **动画效果**：使用 `animated` 属性显示动画效果，提升用户体验。

2. **自定义样式**：使用 `template` 插槽自定义骨架屏样式，尽量靠近真实的 DOM 结构。

3. **防止抖动**：使用 `throttle` 属性延迟占位 DOM 渲染，避免 API 请求过快导致的界面闪烁。

4. **渲染多条数据**：使用 `count` 属性控制渲染多少条假数据，但建议保持最小值以避免性能问题。

5. **Loading 状态**：使用 `loading` 属性控制是否显示骨架屏，配合默认插槽展示真实 UI。
