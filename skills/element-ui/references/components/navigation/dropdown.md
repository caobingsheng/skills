...
### 基础用法
移动到下拉菜单上，展开更多操作。通过组件 `slot` 来设置下拉触发的元素以及需要通过具名 `slot` 为 `dropdown` 来设置下拉菜单。
...
### 触发对象
可使用按钮触发下拉菜单。设置 `split-button` 属性来让触发下拉元素呈现为按钮组。
...
1. **触发方式**：根据使用场景选择 `hover` 或 `click` 触发方式，移动端建议使用 `click`。

2. **按钮组模式**：使用 `split-button` 属性创建按钮组，左侧是功能按钮，右侧是触发下拉菜单的按钮。

3. **菜单项图标**：为菜单项添加图标，提升视觉识别度。

4. **禁用状态**：使用 `disabled` 属性禁用菜单项，提供清晰的视觉反馈。

5. **分割线**：使用 `divided` 属性在菜单项之间添加分割线，提升可读性。
