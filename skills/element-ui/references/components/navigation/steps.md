...
1. **步骤数量**：步骤不得少于 2 步，建议 3-5 步为宜。

2. **激活状态**：使用 `active` 属性设置当前激活步骤，从 0 开始计数。

3. **状态管理**：使用 `process-status` 和 `finish-status` 属性管理步骤状态。

4. **图标使用**：使用 `icon` 属性或插槽自定义图标，提升视觉效果。

5. **简洁风格**：使用 `simple` 属性应用简洁风格，适用于空间有限的场景。
