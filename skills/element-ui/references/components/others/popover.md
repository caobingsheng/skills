...
1. **触发方式**：使用 `trigger` 属性设置触发方式，`click` 适合按钮，`hover` 适合提示。

2. **嵌套内容**：使用默认插槽嵌套表格、表单等复杂内容。

3. **手动控制**：使用 `v-model` 和 `trigger="manual"` 实现手动控制。

4. **宽度设置**：使用 `width` 属性设置弹出框宽度，最小宽度为 150px。

5. **轻量操作**：相比 Dialog 更为轻量，适合简单的确认操作。
