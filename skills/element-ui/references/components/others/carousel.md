...
1. **高度设置**：使用 `height` 属性设置走马灯高度，确保内容正确显示。

2. **自动播放**：使用 `autoplay` 和 `interval` 属性控制自动播放。

3. **指示器位置**：使用 `indicator-position` 属性设置指示器位置。

4. **箭头显示**：使用 `arrow` 属性控制箭头显示时机。

5. **卡片模式**：使用 `type="card"` 启用卡片模式，适合宽度空间充足的场景。
