...
Form 组件用于收集、校验、提交数据，由输入框、选择器、单选框、多选框等控件组成。
...
1. **表单验证**：使用 `rules` 属性传入验证规则，Form-Item 的 `prop` 属性设置为需校验的字段名
2. **数字验证**：数字类型的验证需要在 v-model 处加上 `.number` 修饰符
3. **自定义验证**：可以使用自定义验证函数，callback 必须被调用
4. **嵌套表单**：嵌套在 el-form-item 中的 el-form-item 标签宽度默认为零，需要单独设置 label-width
5. **阻止默认提交**：当表单只有一个输入框时，在 el-form 标签上添加 `@submit.native.prevent` 阻止默认提交
