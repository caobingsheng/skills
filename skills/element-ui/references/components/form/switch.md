...
Switch 开关用于表示两种相互对立的状态间的切换，多用于触发「开/关」。
...
1. **背景色设置**：可以使用 active-color 属性与 inactive-color 属性来设置开关的背景色
2. **文字描述**：使用 active-text 属性与 inactive-text 属性来设置开关的文字描述
3. **扩展 value 类型**：设置 active-value 和 inactive-value 属性，接受 Boolean、String 或 Number 类型的值
4. **图标显示**：设置 active-icon-class 和 inactive-icon-class 可以显示图标，设置此项会忽略文字描述
5. **表单验证**：validate-event 属性控制改变 switch 状态时是否触发表单的校验
