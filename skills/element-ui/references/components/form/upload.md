...
1. **文件限制**：可通过设置 limit 和 on-exceed 来限制上传文件的个数和定义超出限制时的行为
2. **文件验证**：使用 before-upload 限制用户上传的文件格式和大小
3. **阻止删除**：可通过设置 before-remove 来阻止文件移除操作
4. **手动上传**：设置 auto-upload 为 false 可以实现手动上传
5. **自定义上传**：使用 http-request 可以覆盖默认的上传行为，自定义上传实现
